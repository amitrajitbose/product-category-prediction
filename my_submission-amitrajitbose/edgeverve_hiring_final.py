...
model.fit(featuresfinal, labelsfinal)
...
for i in tqdm(dftest.itertuples()):
  ...
  scaled = (i.Inv_Amt-minamt)/(maxamt-minamt)
  f[0].insert(0,scaled)
  x = model.predict(f)[0]
...
submit.to_csv('./submitRT5.csv', index=0)

files.download('submitRT5.csv')